...
{/*{{{*/
  ...
  "error_REGEX_UNEXPECTED_EMPTY_MATCH",
};/*}}}*/
...
{/*{{{*/
  ...
  case c_error_REGEX_WRONG_MATCH_OFFSET:
    ...
    fprintf(stderr," ---------------------------------------- \n");
    break;
  case c_error_REGEX_UNEXPECTED_EMPTY_MATCH:
    fprintf(stderr," ---------------------------------------- \n");
    fprintf(stderr,"Exception: ERROR: in file: \"%s\" on line: %u\n",source.file_name.data,source.source_string.get_character_line(source_pos));
    print_error_line(source.source_string,source_pos);
    ...
    do
    {
      ...
      if (res == 0)
      {
        ...
        if (match.rm_so == match.rm_eo)
        {
          ...
          exception_s::throw_exception(it,module.error_base + c_error_REGEX_UNEXPECTED_EMPTY_MATCH,operands[c_source_pos_idx],(location_s *)it.blank_location);
          return false;
        }
